...
## [2026-01-15] Верстка макета "Займер"

- Реализована верстка одностраничного макета со списком предложений по займам.
- Создан файл `test-verstka.html`.
- Использованы HTML5, CSS3 (переменные) и методология BEM.
- Реализованы компоненты: Header (логотип, кнопки), Сетка карточек МФО (MoneyMan, Webbankir и др.).
- Обеспечена базовая адаптивность для мобильных устройств.

## [2026-01-04] Расширение мира и динамическая камера

- Игровой мир увеличен в 2 раза до 8000x8000 пикселей.
- Реализован динамический зум: камера отдаляется при разгоне, расширяя обзор.
- Физика: Уменьшена резкость рулевого управления, увеличено угловое затухание и инерция кузова.
- Машина стала ощущаться более тяжелой и реалистичной.

## [2026-01-04] Реализация меню настроек

- Создано полноценное меню "Настройки" для редактирования параметров физики в реальном времени.
- Добавлена кнопка в углу экрана, открывающая HTML-интерфейс.
- Поддержано сохранение изменений в `localStorage` через `Storage.js`.
- Параметры применяются мгновенно к `config.carPhysics`.
